 ...
 * `agent_runs` + `run_traces` (one trace doc per run). `pr_intent` is now
 * owned by `modules/intent/repository.ts` (L03) — this class no longer
 * touches it. Workspace scoping is enforced via the PR (which carries workspace_id).
 *
...
export class ReviewRepository {
  ...
  completeAgentRun(
    ...
    values: {
      ...
      tokensIn: number;
      tokensOut: number;
      /** USD spent (provider-reported usage.cost, else the price book); null
       *  when unknown. Null — NOT 0 — on failed/cancelled runs. */
      costUsd?: number | null;
