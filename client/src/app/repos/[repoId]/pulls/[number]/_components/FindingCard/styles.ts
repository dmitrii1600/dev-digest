...
export const s = {
  card: (focused: boolean, sevColor: string, muted: boolean): CSSProperties => ({
    borderRadius: 8,
    // All-longhand, per side. `borderColor` looks like a longhand but is itself
    // a shorthand for the four sides, so pairing it with `borderLeftColor` is
    // exactly the mix React warns about when one of them changes on rerender
    // (it does — `focused` moves with j/k and with the severity filter).
    borderStyle: "solid",
    borderWidth: 1,
